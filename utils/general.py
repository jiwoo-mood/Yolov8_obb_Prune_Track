...
def non_max_suppression_obb_Kpt(prediction, conf_thres=0.45, iou_thres=0.75, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=1500):
    ...
    output_kpt = [torch.zeros((0, 3), device=prediction.device)] * pred_kpt.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # import pdb
        # pdb.set_trace()
        # Apply constraints
        ...
        #对应可用旋转框，挑选可用关键点
        pred_kpt=pred_kpt.transpose(0, -1)[xc[xi]].squeeze(2)
        # x_kpt= x.transpose(0, -1)[xc[xi]]
        # Cat apriori labels if autolabelling
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + 5), device=x.device)
        ...
        output[xi] = x[i]
        output_kpt[xi]=pred_kpt[i]
        # import pdb
        # pdb.set_trace()
    #输出nms之后的旋转框和对应的关键点
    return output,pred_kpt[i]
