...
def create_dataloader(path, imgsz, batch_size, stride, names, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rank=-1, workers=8, image_weights=False, quad=False, prefix='', shuffle=False,is_val=False,use_kpt=False):
    if rect and shuffle:
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',is_val=False,use_kpt=False):
        """
        Returns:
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  
                ...
                #判断标签长度是否包含关键点（only one,如果需要预测更多的关键点此处也需要修改），对关键点的标签做位置缩放
                if len(labels[0])==12:
                    labels[:, [9]] = img_label[:, [9]] * ratio[0] + pad[0]
                    labels[:, [10]] = img_label[:, [10]] * ratio[1] + pad[1]
        ...
        if nl:
            ...
            #不使用高斯窗口函数,判断是否含有关键点
            if len(labels[0])==12:
                labels_obb = np.concatenate((labels[:, :1], rboxes,labels[:, 9:12]), axis=1)
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            ...
            #判断标签长度是否包含关键点（only one,如果需要预测更多的关键点此处也需要修改），对关键点的标签做位置缩放
            if len(labels[0])==12:
                labels[:, [9]] = img_label[:, [9]] + padw
                labels[:, [10]] = img_label[:, [10]] + padh
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            nf = 1  # label found
            with open(lb_file) as f:
                #labels [['1685', '435', '1847', '435', '1847', '500', '1685', '500', 'car', '0', '1692', '465', '2'],id可以直接选0，也可以通过car在cls_name_list中的索引判断，感觉是没写好可以优化。
                ...
                for label in labels:
                    ...
                    #读取gt标签：cls,rbox,kpt
                    if len(label)==13:
                        l_.append(np.concatenate((cls_id, label[:8],label[10:]), axis=None))
                    elif len(label)==10:
            ...
            if nl:
                ...
                # assert len(label) == 13, f'Yolov8-OBB-kpt labels require 13 columns'
                ...
                if len(label)==13:
                    l = np.zeros((0, 12), dtype=np.float32)
                elif len(label)==10:
