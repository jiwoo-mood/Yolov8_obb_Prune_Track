...
from utils.loss import v8DetectionLoss,v8PoseLoss
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        ...
        # model = torch.nn.DataParallel(model,device_ids=[0,1,4,6])
    ...
    if  hyp['use_kpt']:
        compute_loss = v8PoseLoss(model)  # init loss class
    else:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if  hyp['use_kpt']:
            LOGGER.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'cls', 'dfl', 'pose','kobj', 'labels', 'img_size'))
        else:
