...
from utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr,
                           increment_path, non_max_suppression, non_max_suppression_obb, print_args, scale_coords, scale_polys, strip_optimizer, xyxy2xywh,non_max_suppression_obb_Kpt)
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        pred = model(im, augment=augment, visualize=visualize, use_kpt=use_kpt)
        ...
        if  use_kpt:
            pred,pred_kpt = non_max_suppression_obb_Kpt(pred, conf_thres, iou_thres, classes, agnostic_nms, multi_label=True, max_det=max_det)
            pred_kpts = scale_coords(im.shape[2:], pred_kpt, im0s.shape)
        else:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if  use_kpt:
                ...
                for i in range(len(pred_kpts)):
                    x=pred_kpts[i][0].cpu().item()
                    y=pred_kpts[i][1].cpu().item()
                    point_position=(int(x),int(y))
                    print('point_position',point_position)
                    im0=cv2.circle(im0, point_position, radius=6, color=color, thickness=-1)
...
def parse_opt():
    ...
    parser.add_argument('--use_kpt', action='store_true', help='add kpt')
